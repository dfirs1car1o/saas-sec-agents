...
|---|---|---|---|---|---|---|
| BC-001 | P0 | open | Protect public triage path with enforced auth/API gateway policy (not just input validation). | Phase 1 | Platform Security + GIS | Direct app ingress is non-public or strictly gated; JWT/authz enforced; unauthorized requests rejected in RV checks. |
...
| BC-009 | P2 | open | Add runtime policy exception governance API/logging for model/tool policy overrides. | Phase 2/3 | App Engineering + GIS | Exception create/approve/expire path auditable; policy decisions linked to case/correlation ID. |
